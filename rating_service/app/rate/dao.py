...
class RatingDAO(UserActionDAO):
    ...
    @classmethod
    async def rate_movie(cls, rating_form : RateMovieIn, session : AsyncSession | None = None) -> RateMovieOut:
    ...
    async def get_user_movies_rating(cls, session : AsyncSession, user_id : int) -> ShowUserMovieRatingListOut:
        ratings_with_movies = await cls.get_user_movies_rating_or_reviews(session, RatingDB, user_id)
